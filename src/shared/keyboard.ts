...
export let KEY_ACTIONS = new Set(['browser-tools', 'toggle-dark', 'toggle-adblock', 'address', 'command', 'find', 'help', 'sessions', 'tabs', 'bookmarks', 'activity', 'downloads', 'profiles', 'settings', 'reload', 'hard-reload', 'stop', 'new-tab', 'close-tab', 'new-client', 'new-window', 'close-pane', 'close-window', 'rename-window', 'rename-session', 'previous-session', 'next-session', 'next-window', 'previous-window', 'next-pane', 'pane-left', 'pane-down', 'pane-up', 'pane-right', 'toggle-pane-zoom', 'split-right', 'split-down', 'detach', 'back', 'forward', 'zoom-in', 'zoom-out', 'zoom-reset'])
...
// Keep older config files working without exposing duplicate navigation actions.
export let normalizeKeyAction = (action: string) => action === 'next-tab' ? 'next-window' : action === 'previous-tab' ? 'previous-window' : action
