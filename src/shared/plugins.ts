...
export type PluginManifest = { schema_version: 1; id: string; name: string; version: string; actions: PluginAction[]; hooks: PluginHook[] }
export type PluginSettings = Record<string, { enabled: boolean; hooks: boolean }>
export type PluginInfo = { id: string; name: string; version: string; enabled: boolean; hooks: boolean; error?: string; actions: Pick<PluginAction, 'id' | 'title' | 'description'>[] }
