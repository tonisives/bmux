...
export type MemoryPane = {
  paneId: string; windowId: string; sessionId: string; profileId: string
...
type MemoryProcess = {
  ...
  deltaBytes: number | null; webContentsIds: number[]; paneIds: string[]; profileIds: string[]
}
export type MemorySample = {
  capturedAt: number; comparedTo: number | null; totalWorkingSetBytes: number; deltaBytes: number | null
  processes: MemoryProcess[]; panes: (MemoryPane & { processIds: number[] })[]; contents: MemoryOwner[]
...
export let memorySample = (metrics: ProcessMetric[], owners: MemoryOwner[], panes: MemoryPane[], previous?: MemorySample, capturedAt = Date.now()): MemorySample => {
  ...
  let linkedPanes = panes.map(pane => ({ ...pane, processIds: contents.find(owner => owner.webContentsId === pane.webContentsId)?.processIds ?? [] }))
  let processes = uniqueMetrics.map(metric => {
    ...
    let related = linkedPanes.filter(pane => pane.processIds.includes(metric.pid))
    ...
    return {
      ...
      paneIds: related.map(pane => pane.paneId), profileIds: [...new Set(related.map(pane => pane.profileId))],
    }
  ...
  let profiles = [...new Set(panes.map(pane => pane.profileId))].map(profileId => {
  ...
  return { capturedAt, comparedTo: previous?.capturedAt ?? null, totalWorkingSetBytes, deltaBytes: previous ? totalWorkingSetBytes - previous.totalWorkingSetBytes : null, processes, panes: linkedPanes, contents, profiles }
}
...
export let createMemoryDiagnostics = (collect: (previous?: MemorySample) => MemorySample) => {
  ...
  let report = (includeHistory = false) => ({
    ...
    notes: [
      ...
      'Shared renderers are counted once in the total. Profile totals may overlap and exclude processes without a mapped pane.',
