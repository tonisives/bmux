...
export let passwordPopupBounds = (page: FieldBounds, suggestion: PasswordSuggestions, zoom: number): FieldBounds => {
  ...
  let desiredHeight = suggestion.locked ? suggestion.expanded ? 246 : 96 : suggestion.busy || suggestion.message || !suggestion.items.length ? 148 : 46 + Math.min(5, suggestion.items.length) * 54
