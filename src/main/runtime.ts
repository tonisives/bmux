...
export let createRuntime = (dataDirectory: string) => {
  ...
  let installKeys = (contents: WebContents) => {
    contents.on('before-input-event', (event, input) => {
      ...
      if (doubleTap.update(input, clickSettings.enabled ? clickSettings.doubleTapModifier : null)) { event.preventDefault(); void activateClickMode().catch(reportError); return }
    ...
    contents.on('before-mouse-event', (event, mouse) => {
      if (mouse.type === 'mouseDown' || mouse.type === 'mouseWheel') {
        let client = [...clients].find(([, live]) => live.window.isFocused() && (live.chrome.webContents === contents || live.permissionPopup.webContents === contents || [...tabs.values()].some(tab => tab.contents === contents && tab.parent === live.window)))
  ...
  let createClient = async (sessionId: string, restored?: Client, activate = true) => {
    ...
    let resizeChrome = () => { clickMode.cancelClient(client.id); let bounds = window.getContentBounds(); chrome.setBounds({ x: 0, y: 0, width: bounds.width, height: bounds.height }); client.width = bounds.width; client.height = bounds.height; save() }
