...
let activate = async () => {
  ...
  await chrome.evaluate(async () => { let state = await window.browmux.state(); await window.browmux.command({ method: 'activate-client', args: { client: state.clientId } }) })
...
try {
  ...
  if (process.env.BROWMUX_TEST_SECOND_URL) {
    let secondUrl = process.env.BROWMUX_TEST_SECOND_URL
    ...
    let urls = await chrome.evaluate(async () => (await window.browmux.state()).model.sessions[0].windows.map(window => window.panes[0].tabs[0].url))
  ...
  let chrome = application.context().pages().find(page => page.url().endsWith('/renderer/index.html'))
  let selection = await chrome?.evaluate(async () => { let state = await window.browmux.state(); return { focusedClientId: state.focusedClientId, clients: state.model.clients, windows: state.model.sessions[0].windows.map(window => ({ id: window.id, panes: window.panes.map(pane => ({ id: pane.id, tab: pane.activeTabId })) })) } })
