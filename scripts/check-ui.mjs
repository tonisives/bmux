...
let server = http.createServer((request, response) => {
  ...
  response.end('<!doctype html><title>bmux URL fixture</title><style>body{margin:0;background:#e8eef8;color:#173353;font:20px sans-serif}h1{margin:0;padding:30px;background:#173353;color:white}p{padding:10px 30px}</style><h1>bmux URL fixture</h1><p>This page was opened from a pane address bar.</p>')
})
...
try {
  ...
  let secondPane = await chrome.evaluate(async () => {
    let state = await window.bmux.state(), client = state.model.clients.find(client => client.id === state.clientId)
    return window.bmux.command({ method: 'split-window', args: { pane: client.paneId } })
  })
