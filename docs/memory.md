...
Use the page context menu's “Keep Page Loaded” toggle for a window that must retain its in-memory state. Protected windows load on launch and are exempt from idle unloading. Agents can set the same persistent flag with `tab.keep-alive` and `{ "tab": "tab_id", "enabled": true }`. The `tab` command and ID names remain for API compatibility. Idle unloading restores navigation history and scroll position when Electron can recover them, but does not preserve arbitrary JavaScript state.
...
Each process is counted once in `totalWorkingSetBytes`. Windows sharing a renderer
list the same PID; do not count that process separately for each window. Profile
totals can overlap if a process maps to more than one profile, and exclude
processes without mapped windows. Browser, GPU, utility, and unmapped processes are
