...
Run the import again to update bookmarks without duplicating profiles or sessions. Name collisions create a separate name such as `bot (Brave)`. bmux backs up existing state and bookmark files before import. Imported bookmarks are stored in `~/.config/bmux/bookmarks.yaml` (see [Bookmarks](bookmarks.md)).
