...
test('fuzzy command search matches subsequences and independent terms while preferring command names', () => {
  ...
  expect(searchCommands(COMMANDS, 'tabs')[0].command).toBe('tabs')
...
test('finder includes active shortcuts, enabled plugin actions, and deduplicated recent commands', () => {
  ...
  let results = searchCommands(entries, '', ['tabs', 'browser-tools', 'tabs'])
  expect(results.slice(0, 2).map(entry => entry.command)).toEqual(['tabs', 'browser-tools'])
  expect(results.filter(entry => entry.command === 'tabs')).toHaveLength(1)
})

test('typed command arguments and URL navigation remain literal', () => {
  for (let value of ['new-window -n "two words"', 'open https://example.test/path', 'dark off --scope profile', 'https://example.test/', 'example.test', 'tab select -t 0', 'split --profile bot']) expect(literalCommand(value)).toBe(true)
