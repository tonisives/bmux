...
test('pane shortcuts move the macOS pointer into page content, including zoomed panes', async () => {
  ...
  try {
    ...
    await rpc('navigate', { tab: left.activeTabId, url: `${url}/left` })
    ...
    await rpc('wait', { tab: lower.activeTabId, selector: 'h1' })
    ...
    await key('Down', [])
    await expect.poll(() => rpc('eval', { tab: lower.activeTabId, expression: 'scrollY' })).toBeGreaterThan(0)
    await rpc('toggle-pane-zoom', { client: client.id })
