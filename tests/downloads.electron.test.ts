...
test('download manager controls real transfers and isolates profiles', async () => {
  ...
  await start('slow')
  await expect(downloadButton.locator('[data-download-icon="progressing"]')).toBeVisible()
  await expect(downloadButton).not.toContainText('downloads')
  ...
  await expect.poll(async () => Number(await downloadButton.locator('[data-download-icon="progressing"]').getAttribute('data-download-progress'))).toBeGreaterThan(0)
  await expect(downloadButton).toHaveAttribute('title', /\d+%/)
  await row.getByRole('button', { name: 'Pause', exact: true }).click()
  await expect(row).toContainText('Paused')
  await expect(downloadButton.locator('[data-download-icon="idle"]')).toBeVisible()
  ...
  expect(path.basename(record.path)).toBe('complete.bin')
  expect((await fs.stat(record.path)).size).toBe(1024)
  await start('complete')
  let duplicate = chrome.getByRole('article', { name: 'complete (1).bin', exact: true })
  await expect(duplicate).toContainText('Completed')
  let duplicateRecord = (await downloads()).find(item => item.name === 'complete (1).bin')
  expect(path.basename(duplicateRecord.path)).toBe('complete (1).bin')
  expect((await fs.stat(duplicateRecord.path)).size).toBe(1024)
  await fs.unlink(record.path)
  await fs.unlink(duplicateRecord.path)
  ...
  await duplicate.getByRole('button', { name: 'Show in Finder' }).click()
  await expect(downloadButton).toHaveCount(0)
  await start('broken')
