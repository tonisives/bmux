...
test('pane address bars navigate independently and leave window switching available', async () => {
  ...
  let suggestions = secondPane.getByRole('listbox', { name: 'Address suggestions' })
  ...
  expect(suggestionBounds!.width).toBeCloseTo(contentWithSuggestions!.width * .9, 0)
  expect(suggestionBounds!.height).toBeCloseTo(contentWithSuggestions!.height * .9, 0)
