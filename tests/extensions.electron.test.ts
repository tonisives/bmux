...
test('loads an extension per profile, opens its sandboxed popup, restores and removes it', async ({}, testInfo) => {
  ...
  try {
    ...
    expect((await rpc('extension.list', { profile: other.id })).available).toEqual([{ name: installed.name, version: installed.version, path: installed.path }])
    ...
    let currentState = await rpc('state')
    let current = currentState.model.clients.find((client: { id: string }) => client.id === currentState.focusedClientId)
    ...
    await rpc('new-window', { session: current.sessionId, profile, url, client: current.id })
    await expect.poll(() => application!.context().pages().some(page => page.url() === url)).toBe(true)
    await rpc('extension.open', { profile, id: installed.id })
    await expect(popup.locator('#active-tab')).toHaveText(url)
    ...
    await focusClient()
    await expect.poll(async () => (await rpc('state')).focusedClientId).toBeTruthy()
    await rpc('select-window', { client: current.id, window: current.windowId })
    await rpc('extension.open', { profile, id: installed.id })
    await expect(popup.locator('#active-tab')).toHaveText(nextUrl)
    ...
    await sharingPanel.getByRole('button', { name: 'Enable Fixture extension' }).click()
    await expect(sharingPanel.getByRole('button', { name: /^Fixture extension/ })).toBeVisible()
    ...
    await expect(sharingPanel.getByRole('button', { name: /Remove/ })).toHaveCount(0)
    await expect(sharingPanel.getByRole('switch', { name: 'Enable Fixture extension' })).toBeChecked()
    await sharingPanel.getByRole('button', { name: 'Details for Fixture extension' }).click()
    let details = sharingPanel.getByRole('region', { name: 'Extension details' })
    await expect(details).toContainText('A fixture for extension details.')
    await expect(details).toContainText('1.1')
    await expect(details).toContainText(installed.id)
    await expect(details).toContainText('storage, tabs')
    await expect(details).toContainText('http://127.0.0.1/*')
    await expect(details).toContainText('bookmarks (not granted)')
    await expect(details.getByRole('button', { name: 'Back to extensions' })).toBeFocused()
    await expect(details.getByRole('region', { name: 'Danger zone' }).getByRole('button', { name: 'Remove extension' })).toBeVisible()
    await sharingPanel.screenshot({ path: testInfo.outputPath('extension-details.png') })
    await details.getByRole('button', { name: 'Options', exact: true }).click()
    ...
    await chrome.getByRole('button', { name: 'Extensions', exact: true }).click()
    await sharingPanel.getByRole('switch', { name: 'Enable Fixture extension' }).click()
    await expect(sharingPanel.getByRole('switch')).not.toBeChecked()
    await expect(sharingPanel).not.toContainText('Disabled')
    ...
    await sharingPanel.getByRole('button', { name: 'Details for Fixture extension' }).click()
    await sharingPanel.getByRole('button', { name: 'Remove extension', exact: true }).click()
