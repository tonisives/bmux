...
it('loads macOS defaults, remaps shortcuts and validates YAML', () => {
  ...
  expect(matchesBinding('CmdOrCtrl+Shift+T', { key: 'T', meta: true, shift: true }, 'darwin')).toBe(true)
  expect(matchesBinding('CmdOrCtrl+Shift+T', { key: 'T', control: true, shift: true }, 'linux')).toBe(true)
  expect(matchesBinding('CmdOrCtrl+Shift+T', { key: 'T', control: true, shift: true }, 'win32')).toBe(true)
