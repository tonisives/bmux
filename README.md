...
The UI check types `https://example.com/` into the URL prompt, submits Enter, verifies the native page is attached and visible, and saves `artifacts/url-opened.png`. `pnpm debug:ui` runs the same check and leaves the debug window open with temporary profiles. Set `BMUX_TEST_URL` to check a different URL.
